...
import { IUser, IUserCreateParams } from '@codeclusters/types'
...
export const userService = {
  ...
  getUsers: async (): Promise<IUser[] | undefined> => {
    const users = (await dbService.queryMany(
      'SELECT id, name, email, role, student_id FROM app_user'
    )) as IUser[]
    return users
  },
  createUser: async (params: IUserCreateParams): Promise<IUser | undefined> => {
    const user = (await dbService.queryOne(
      ...
      [params.name, params.email, params.password, params.role, params.student_id]
    )) as IUser | undefined
    return user
  },
}
