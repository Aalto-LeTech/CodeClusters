...
export const REVIEW_LIST_QUERY_PARAMS = Joi.object({
  ...
  statuses: Joi.array()
    .items(Joi.string().valid(EReviewStatus.SENT, EReviewStatus.PENDING))
    .min(1)
    .required(),
})
...
export const getReviews = async (
  ...
  res: AuthResponse<any, IReviewListQueryParams>,
  next: NextFunction
) => {
  try {
    const reviews = await reviewService.getReviews(res.locals.queryParams)
    const reviewSubmissions = await reviewService.getReviewSubmissions(res.locals.queryParams)
...
export const getUserReviews = async (
  req: IAuthRequest<{}, { user_id: string }>,
  res: Response,
  next: NextFunction
) => {
  try {
...
export const createReview = async (
  req: IAuthRequest<IReviewCreateParams>,
  res: Response,
  next: NextFunction
) => {
  try {
...
export const acceptPendingReviews = async (
  req: IAuthRequest<IAcceptReviewsParams>,
  res: Response,
  next: NextFunction
) => {
  try {
...
export const updateReview = async (
  req: IAuthRequest<IReview, { review_id: string }>,
  res: Response,
  next: NextFunction
) => {
  try {
...
export const deleteReview = async (
  req: IAuthRequest<{}, { review_id: string }>,
  res: Response,
  next: NextFunction
) => {
  try {
