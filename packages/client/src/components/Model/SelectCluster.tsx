...
const SelectClusterEl = inject((stores: Stores) => ({
  ...
  observer((props: IProps) => {
    const { className, activeCluster, currentClusters, setActiveCluster, resetActiveCluster } =
      props
    ...
    function handleClickIcon(direction: 'left' | 'right') {
      ...
      if (direction === 'left' && clusterKeys.length > 0 && currentIndex !== 0) {
        ...
        currentIndex !== clusterKeys.length - 1
      ) {
        setActiveCluster!(clusterKeys[currentIndex + 1])
      }
    }
    ...
    return (
      <Container className={className}>
        <SearchHeader>
          ...
          <ParamsHeaderButtons>
            <Button intent="transparent" onClick={handleResetActiveCluster}>
              Reset
            </Button>
          </ParamsHeaderButtons>
        </SearchHeader>
