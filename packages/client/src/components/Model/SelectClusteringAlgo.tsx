...
const SelectClusteringAlgoEl = (props: IProps) => {
  ...
  const [clusteringAlgo, setClusteringAlgo] = useState<ClusteringAlgoType>(
    getValues('selected_clustering_algo')
  )
  const clusteringInfo = useMemo(
    () => CLUSTERING_INFO.find((c) => c.key === clusteringAlgo),
    [clusteringAlgo]
  )
    ...
    <Container className={className}>
      ...
      <Body>
        <FormField>
          ...
          />
          <input type="hidden" name="selected_clustering_algo" ref={register} />
          <ClusteringDescription>{clusteringInfo?.text}</ClusteringDescription>
    ...
    <ClusteringFields visible={visible}>
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.DBSCAN?.min_samples && 'Min samples should be higher than 0'}</Error>
      </FormField>
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.DBSCAN?.eps && 'DBSCAN eps must be between 0-1'}</Error>
      </FormField>
    </ClusteringFields>
  )
}
    ...
    <ClusteringFields visible={visible}>
      ...
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.HDBSCAN?.min_samples && 'Min samples should be higher than 0'}</Error>
      </FormField>
    </ClusteringFields>
  )
}
    ...
    <ClusteringFields visible={visible}>
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.OPTICS?.min_samples && 'Min samples should be higher than 0'}</Error>
      </FormField>
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.OPTICS?.max_eps && 'Max eps should be >0 or empty'}</Error>
      </FormField>
    </ClusteringFields>
  )
}
    ...
    <ClusteringFields visible={visible}>
      <FormField>
        ...
        <Input
          ...
          ref={register}
        />
        <Error>{errors?.KMeans?.k_clusters && 'K-means clusters should be >1'}</Error>
      </FormField>
    </ClusteringFields>
  )
}
