...
import { INgramParams, TokenSetType, NgramModelId } from '@codeclusters/types'
...
const CLUSTERING_OPTIONS = ['DBSCAN', 'HDBSCAN', 'OPTICS', 'KMeans']
const DIM_VISUALIZATION_OPTIONS = ['TSNE', 'UMAP']
...
const resolver = (data: INgramFormParams, validationContext?: object) => {
  ...
  const errors =
    error?.details.reduce((acc, currentError) => {
...
function generateDefaultData(initialData?: INgramParams) {
  return {
    ...
    DBSCAN:
      initialData?.clustering_params?.name !== 'DBSCAN'
        ? {
            min_samples: 5,
            eps: 0.25,
          }
    ...
    HDBSCAN:
      initialData?.clustering_params?.name !== 'HDBSCAN'
        ? {
            min_cluster_size: 2,
            min_samples: 5,
        ...
        : initialData?.clustering_params,
    OPTICS:
      initialData?.clustering_params?.name !== 'OPTICS'
        ? {
            min_samples: 5,
            max_eps: undefined,
          }
        : initialData?.clustering_params,
    KMeans:
      initialData?.clustering_params?.name !== 'KMeans'
        ? {
            k_clusters: 2,
          }
        : initialData?.clustering_params,
    TSNE:
      initialData?.dim_visualization_params?.name !== 'TSNE'
        ? {
    ...
    UMAP:
      initialData?.dim_visualization_params?.name !== 'UMAP'
        ? {
            n_neighbors: 30,
            min_dist: 0.0,
          }
...
const NgramParametersFormEl = observer(
  forwardRef((props: IProps, ref) => {
    ...
    function normalizeFormData(data: INgramFormParams): INgramParams {
      const {
        ...
        max_ngrams,
        random_seed,
        selected_clustering_algo,
        selected_dim_visualization,
      } = data
      const payload = {
        ...
        >
          <TopRow>
            <FormField>
              ...
              />
              <input type="hidden" name="token_set" ref={register} />
            </FormField>
            <FormField>
              <label>N-grams</label>
              <MinMaxGram>
              ...
              <Error>{errors.min_ngrams && 'Min n-gram must be 1-20'}</Error>
              <Error>
            ...
            <FormField className="random_seed">
              ...
              <Input
                ...
                ref={register}
              />
              <Error>{errors.random_seed && '-1 or empty for no seed, otherwise >=0'}</Error>
            </FormField>
          </TopRow>
          <MiddleRow>
          ...
          {onSubmit && (
            <Buttons>
              ...
              <Button intent="transparent" onClick={handleCancel}>
                Cancel
              </Button>
            </Buttons>
          )}
