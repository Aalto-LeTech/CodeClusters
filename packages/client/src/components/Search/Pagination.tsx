  ...
  observer((props: IProps) => {
    ...
    return (
      <Container className={`${className} ${isHidden && 'hidden'}`}>
        <Icon button onClick={() => handleClickIcon('left')}>
          <FiChevronLeft size={20} />
        </Icon>
        <PageList>
          {leftDots && (
            <PageItem>
              <PageButton selected={false} onClick={() => handleClickPage(1)}>
                {1}
              </PageButton>
            </PageItem>
          )}
          ...
          {Array(pages)
            ...
            .map((_, i) => (
              <PageItem key={i}>
                <PageButton
                  selected={leftPages + i === currentPage}
                  onClick={() => handleClickPage(leftPages + i)}
                >
                  {leftPages + i}
                </PageButton>
              </PageItem>
          ...
          {rightDots && (
            <PageItem>
              <PageButton selected={false} onClick={() => handleClickPage(maxPages)}>
                {maxPages}
              </PageButton>
            </PageItem>
          )}
        </PageList>
        <Icon button onClick={() => handleClickIcon('right')}>
