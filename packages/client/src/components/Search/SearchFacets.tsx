...
const SearchFacetsEl = inject((stores: Stores) => ({
  ...
  observer((props: IProps) => {
    const { className, currentMetricsFacets, currentTokensFacets, resetFacets } = props
          ...
          <FacetGroupHeader>
            <TitleWrapper>
              ...
              <Tooltip title="Metrics facets" size={20}>
                <TooltipText>
                  <p>
                    Facets are a feature of Lucene to easily group and filter search results. For
                    any indexed value, such as metrics in this case, you can categorize them based
                    on the counts or ranges of the values. The resulting buckets can be then
                    selected by checking their checkbox. The middle column is the value of the
                    bucket and the right column the count. Current Java metrics are generated using
                    Checkstyle metrics.
                  </p>
                  <p>
                    Whenever you change the facet values, you must rerun the search. Closing a facet
                    will delete its values. If you don't specify a range, the results are the counts
                    of individual values (which is why all metrics are integers). Using a range you
                    can specify its start, end and the gap which is the interval the results are
                    bucketed into.
                  </p>
                  <div>
                    <a
