...
export const courseService = {
  getCourses: async () : Promise<ICourse[] | undefined> => {
    return await dbService.queryMany<ICourse>(`
      SELECT *
      FROM (
        SELECT course.course_id, course.name, organization, array_agg(course_students.student_id) as student_ids
        FROM course LEFT JOIN course_students ON course.course_id = course_students.course_id
        GROUP BY (course.course_id, course.name, organization)
      ) c
      LEFT JOIN (
